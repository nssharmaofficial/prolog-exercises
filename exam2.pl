...
element_at(X, [X|_], 1).  % we found the element
element_at(X, [_|T], K) :- element_at(X, T, K2),
                           K is K2+1.
                           
% But we want only to find the first appearance, 
% so we add a cut (!) to avoid backtracking.
first_occurence_at(X, [X|_], 1) :- !.
first_occurence_at(X, [_|T], K) :- first_occurence_at(X, T, K2),!,
                                   K is K2+1. 
                                   
first_pair(X,Y,L,K) :- first_occurence_at(Y,L,KY),
    				   KX is KY-1,
    				   first_occurence_at(X,L,KX),
    				   K is KX.

% delete K-th item from a list
delete_kth(1,[_|T],T).
delete_kth(K,[X|Y],[X|R]):- K1 is K-1,
                            delete_kth(K1,Y,R).
